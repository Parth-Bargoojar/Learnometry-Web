...
const trustBadges: { icon: LucideIcon; label: string }[] = [
  { icon: ShieldCheck, label: "CBSE, NTA & NMC Exam Patterns" },
  { icon: Calculator, label: "Deterministic Scoring (0% AI Guesswork)" },
  { icon: Sparkles, label: "30 Free Monthly Credits Forever" },
];
...
export function Hero() {
  ...
  return (
    <section
      className={`${sectionShell} relative overflow-hidden border-b-2 border-ink bg-background`}
    >
      <div
        aria-hidden="true"
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-14">
          <div className="flex flex-col items-start gap-6">
            ...
            <div id="waitlist" className="w-full scroll-mt-28">
              {isJoined ? (
                <div className="flex w-full max-w-lg flex-col gap-3 rounded-card-sm border-2 border-ink bg-emerald-50 p-4 text-ink shadow-brutal">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-600">
              ...
              <a
                href="#how-it-works"
                className="font-semibold text-ink underline decoration-primary decoration-2 underline-offset-4 transition-colors hover:text-primary-text"
              >
                See how it works ↓
              </a>
            </div>
            ...
            <ul className="flex flex-wrap gap-2.5">
              {trustBadges.map((badge) => (
                <li
                  key={badge.label}
                  className="inline-flex items-center gap-1.5 rounded-full border-2 border-ink bg-surface px-3 py-1.5 text-xs font-semibold text-ink shadow-brutal-sm"
                >
                  <badge.icon
                    aria-hidden="true"
      ...
      <article className="rounded-card-lg border-2 border-ink bg-surface p-6 sm:p-7 shadow-brutal-lg">
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-btn border border-ink/20 bg-slate-50 px-3.5 py-2.5 text-xs">
          <span className="flex items-center gap-1.5 font-medium text-slate-700">
            ...
            <span>Saved <strong>4.2 hours</strong> of blind chapter revision</span>
          </span>
